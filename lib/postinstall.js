...
function print({
  ...
  log.info('  3. Commit:  git add . && git commit -m "Add @insynq/agent-blueprint v' +
           manifest.version + '"');
  log.blank();

  log.info('Framework ownership:');
  log.info('  Framework owns:  .claude/commands/*, docs/OpenClaw KBs/');
  log.info('  You own:         workspace/ content, deploy/ values, your scripts');
