...
// @insynq/agent-blueprint — `init` entrypoint.
//
...
const HELP = `\
@insynq/agent-blueprint — install the framework into an existing repo.

Usage:
  npx @insynq/agent-blueprint init [flags]
...
async function main() {
  ...
  try {
    ...
    log.error(
      ...
      'Re-run `npx @insynq/agent-blueprint init` to retry.'
    );
    return 1;
  }
