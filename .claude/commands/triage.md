---
description: Triage a stale backlog (PRs / branches / open work items) into action buckets with judge-verified verdicts and a fail-loud coverage tally
argument-hint: "[the backlog to triage — a list of PRs/branches/items, or a source to discover them from (e.g. 'open PRs on origin'); discovers via git/gh if omitted]"
---
...
Work through the steps below **in order**. The item enumeration (Step 1) and the coverage tally (Step 6) are the two mechanical musts — everything between them is judgment. Do not skip the tally even if every item looks obviously bucketed.
...
- **Unanimity earns nothing.** Agreement between investigator and judge is NOT promoted to "verified" — correlated agents sharing a seed error launder a false verdict into false confidence (`docs/LESSONS.md` `[PROCESS-4]`; the F1 false-negative case). A unanimous verdict is *un-contested*, which is all the ledger claims for it — not *independently confirmed*.

## Step 5: Coverage guard — MANDATORY, fail loud ([PROCESS-3])
...
- **Any dropped or failed investigator/judge = verdict `UNVERIFIED`** for that item, surfaced **loudly** as its own row in the output table. A subagent that died, timed out, or returned nothing does NOT get omitted — it gets an `UNVERIFIED` row so the gap is visible, never silent (`docs/LESSONS.md` `[PROCESS-3]`: a schema-forced or dropped fan-out lane must be logged, never allowed to masquerade as complete coverage).
