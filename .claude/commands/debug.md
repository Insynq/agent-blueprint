...
- **Third hypothesis just failed (3 strikes)** → **before** accepting "wrong model of the system," run a **mandatory independent refutation** — the falsification seam fires here *unconditionally*, because three strikes is the moment confirmation bias is most entrenched. Spawn ONE fresh `Explore` agent (a context that did NOT form the hypotheses), given only the latest root-cause hypothesis + its `file:line`, with the mandate: *"Prove this root cause WRONG against primary source — find evidence it is not the cause, or an alternative live code path that is. Quote it."*
  - **Skeptic refutes it** (finds contradicting source or a live alternative path) → the real cause is elsewhere; re-investigate with that evidence. This is a redirect, **not** a 4th fix attempt.
  - **Skeptic cannot refute it** after a real search → the *"the problem is the model of the system, not the fix"* conclusion is now **earned, not self-asserted**. **STOP. Do not re-spawn a 4th debug agent.** Re-examine the assumptions behind the Step 1 characterization and the layer choice. State to the user: *"3 fixes failed and an independent skeptic could not refute the latest root cause — the problem is likely the model of the system, not the fix."* Recommend writing a spec doc + running `/brainstorm` or `/plan-review` to question the architecture, rather than another fix.
