...
```
/update-framework — pull canonical agent-blueprint updates into this project.
...
What this command will do:
  ...
  2. Fetch the canonical tarball at install-version AND target-version
     from github.com/Insynq/agent-blueprint releases (immutable tags).
...
Run `npx @insynq/agent-blueprint init` first to install the framework,
...
re-run the installer at the version you intended (`npx @insynq/agent-blueprint
...
- `canonical_url` — for constructing GitHub API endpoints (REQUIRED — fall back to a hardcoded `https://github.com/Insynq/agent-blueprint` and warn the user if missing).
...
Parse `canonical_url` to extract owner and repo. Expected pattern: `https://github.com/<owner>/<repo>` or `github.com/<owner>/<repo>`. Use a simple regex; if parse fails, warn and fall back to `Insynq` / `agent-blueprint`.
...
4. Extract:
   ...
   `--strip-components=1` removes the GitHub-prefix top-level dir (e.g., `Insynq-agent-blueprint-abc123/`).
...
```
/update-framework — agent-blueprint v<install-version> → v<target-version>
...
Updated: agent-blueprint v<install-version> → v<target-version>
