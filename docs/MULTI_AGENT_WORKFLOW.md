...
```
docs/plans/skill-rework/
├── phase-plan.md
├── worker-1-mcporter-validation.md
├── worker-2-skill-frontmatter-audit.md
└── worker-3-bootstrap-cap-cleanup.md
```
...
Examples: `Worker 1 | Fix mcporter env-var resolution`, `Worker 2 | Add new skill for triage`. The number is for disambiguation; the task name is the human-readable hint.
...
```
Workers in flight:
- Worker 2 | New skill registration — implementing (Phase 7)
- Worker 3 | MCP server build — blocked on user input (see Worker 3 tab)

Workers wrapped:
- Worker 1 | mcporter audit — implementation done, integrated
