...
- **Persist scaffolding IDs to the update-safe config surface, not the code.** When the agent *creates* cloud artifacts at first-run (Drive folder, calendar, spreadsheet), store each real ID in the runtime-mutable config store (TASK-QUEUE.md, or a plugin config-home that survives updates) — a pointer-to-truth like the SCHEMA bullet above, never a copy of contents. See `OC_KB_16` (persist-scaffolding-IDs corollary) for the full pattern and its fewer-calls / idempotency payoffs.
