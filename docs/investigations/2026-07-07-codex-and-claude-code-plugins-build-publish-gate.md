...
- **Hooks are discovered at startup — a full restart is required.** After installing/updating a plugin, the *current* session won't see its hooks; you must fully quit and relaunch Codex (a new thread is not enough). A "No hooks found" panel is usually a session that started before the plugin's hooks existed. On the clean restart, Codex prints a review warning; approve via `/hooks`.
- **`/hooks` is a CLI command and may not surface in the app GUI.** In one build the app's Hooks panel stayed empty / `/hooks` reported "no commands," so the trust step couldn't be completed there. Diagnostic that isolates it: the installed cache (`~/.codex/plugins/cache/<mkt>/<plugin>/<ver>/`) had the manifest `hooks` field + a valid `hooks/hooks.json` + `[features] hooks` enabled, yet no prompt — i.e. purely the trust/restart gap, not packaging.
- **Backstop — skills load the spine anyway.** Because every `SKILL.md` references `AGENTS.md`, the safety spine loads whenever a skill runs, *even if the hook is never trusted* (observed: correct permission-first / confirm-before-send behavior with an untrusted hook). So the SessionStart hook is belt-and-suspenders, not the only path. If you need it unconditional and the trust UI won't cooperate, the fallback is `~/.codex/AGENTS.md` (auto-loaded, no trust) — but that's **global to every Codex session**, so only use it on a machine dedicated to this agent.
